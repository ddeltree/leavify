...
type NoFunctionKeys<T> =
  Entries<T>[keyof T] extends infer Entry ?
    // eslint-disable-next-line @typescript-eslint/ban-types
    Entry extends readonly [keyof T, Function] ? never
    : Entry extends readonly [keyof T, unknown] ? Entry[0]
    : never
  : never;
...
type RecursivePartial<T> =
  ...
  : {
      [K in NoFunctionKeys<T>]?: T[K] extends (infer U)[] ?
        RecursivePartial<U>[]
      : T[K] extends object | undefined ? RecursivePartial<T[K]>
      : T[K];
    };
