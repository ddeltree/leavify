...
const sites: Record<string, string> = {
  "get(m,'|')": `import { get } from '@accessors';\n@MODEL@declare const m: M;\nconst v = get(m, '${CURSOR}');\n`,
  "set(m,'|')(v)": `import { set } from '@accessors';\n@MODEL@declare const m: M;\nset(m, '${CURSOR}')('x');\n`,
  "PickLeaves<M,'|'>": `import type { PickLeaves } from '@typings';\n@MODEL@type P = PickLeaves<M, '${CURSOR}'>;\n`,
  "pickLeaves(m,'|')": `import { pickLeaves } from '@accessors';\n@MODEL@declare const m: M;\npickLeaves(m, '${CURSOR}');\n`,
  "mask<M>().pick('|')": `import { mask } from '@accessors';\n@MODEL@mask<M>().pick('${CURSOR}');\n`,
  "mask<M>().pick(..).omit('|')": `import { mask } from '@accessors';\n@MODEL@mask<M>().pick('g0.f0').omit('${CURSOR}');\n`,
};

for (const [site, tpl] of Object.entries(sites)) {
  for (const g of SIZES) {
    const leaves = g * (2 * FIELDS + 2);
    const snippet = tpl.replace('@MODEL@', modelSource('M', g, FIELDS));
